...
bool cppLint::runThroughString(unsigned int& c, string& line, ostream &os){
...
bool cppLint::runThroughAngleBracket(unsigned int& c, string& line, ostream &os){
    //if hits a string "" go until completion
    if(line.at(c)=='<'){
        os<<line.at(c);
        while(line.at(c)!='>'){
...
bool cppLint::runThroughChar(unsigned int& c, string& line, ostream &os){
    //if hits a string '' go until completion
    if(line.at(c)=='\''){
        os<<line.at(c);
        while(line.at(c)!='\''){
...
bool cppLint::runThroughSingleLineComment(unsigned int&c, string& line, ostream &os){
...
bool cppLint::runThroughWhiteSpace(unsigned int&c, string&line){
    bool space = false;
...
// EFFECT: Add indent
void cppLint::indent(ostream &os){
...
bool cppLint::runThroughWords(unsigned int&c, string& line, ostream&os){
...
bool cppLint::includeStatement(unsigned int& c, string& line, ostream &os){
    assert(c==0);
...
//EFFECT: Indent {}
bool cppLint::curlOpen(unsigned int& c, string& line, ostream &os, bool atNewLine){
...
bool cppLint::curlClose(unsigned int&c, string& line, ostream &os, bool atNewLine){
    if(line.at(c)!='}') return false;
    indLev-=1;
    if(!atNewLine) os<<"\n";
    indent(os);
...
bool cppLint::semicolon(unsigned int&c, string& line, ostream&os){
    if(line.at(c)!=';') return false;
    c++;
    os<<";\n";
    return true;
...
void cppLint::run(istream &is, ostream &os) {
